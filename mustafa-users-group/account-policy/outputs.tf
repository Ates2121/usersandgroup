output "this_caller_identity_account_id" {
  ...
  value       = element(concat(data.aws_caller_identity.this.*.account_id, ["owner-id"]), 0)
}

output "this_caller_identity_arn" {
  description = "The AWS ARN associated with the calling entity"
  value       = element(concat(data.aws_caller_identity.this.*.arn, ["owner-arn"]), 0)
}

output "this_caller_identity_user_id" {
  description = "The unique identifier of the calling entity"
  value       = element(concat(data.aws_caller_identity.this.*.user_id, ["owner-id"]), 0)
}
...
output "this_iam_account_password_policy_expire_passwords" {
  ...
  value       = element(concat(aws_iam_account_password_policy.this.*.expire_passwords, ["90"]), 0)
}
