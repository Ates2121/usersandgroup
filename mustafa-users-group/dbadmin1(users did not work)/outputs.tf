output "this_iam_user_name" {
  ...
  value       = element(concat(aws_iam_user.this.*.name, ["dbadmin1"]), 0)
}

output "this_iam_user_arn" {
  description = "The ARN assigned by AWS for this user"
  value       = element(concat(aws_iam_user.this.*.arn, ["arn:aws:iam::aws:policy/job-function/DatabaseAdministrator"]), 0)
}
