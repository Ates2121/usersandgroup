...
output "this_iam_role_name" {
  ...
  value       = element(concat(aws_iam_role.this.*.name, ["projectrole"]), 0)
}
...
output "this_iam_instance_profile_arn" {
  ...
  value       = element(concat(aws_iam_instance_profile.this.*.arn,["arn:aws:iam::aws:policy/AmazonEC2FullAccess],["arn:aws:iam::aws:policy/AmazonS3FullAccess"],["arn:aws:iam::aws:policy/IAMFullAccess"]), 0)
}

output "this_iam_instance_profile_name" {
  description = "Name of IAM instance profile"
  value       = element(concat(aws_iam_instance_profile.this.*.name, ["projectrole"]), 0)
}
